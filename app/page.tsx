...
const content = {
  en: {
    ...
    toggleLabel: "Español",
    cmaTitle: "Get Your Instant Home Valuation",
    cmaSubtitle: "Enter your address below for a custom market report",
    featuredTitle: "Featured Office Listings",
    featuredSubtitle: "Explore premier properties currently on the market",
    viewAllListings: "View All Properties",
    links: [
      {
  ...
  es: {
    ...
    toggleLabel: "English",
    cmaTitle: "Obtén el Valor de tu Propiedad",
    cmaSubtitle: "Ingresa tu dirección para ver el estimado del mercado",
    featuredTitle: "Propiedades Destacadas de la Oficina",
    featuredSubtitle: "Explora las propiedades exclusivas disponibles",
    viewAllListings: "Ver Todas las Propiedades",
    links: [
      {
...
export default function LinkInBioPage() {
  ...
  useEffect(() => {
    ...
    // Shadow DOM style injector using MutationObserver to ensure it hits right when RealScout renders
    const injectShadowStyles = () => {
      ...
      if (el && el.shadowRoot) {
        if (!el.shadowRoot.querySelector("#rs-force-2col")) {
          ...
          style.textContent = `
            /* Lock each card to exactly 50% width */
            .swiper-slide,
            [class*="slide"],
            [class*="card"] {
  ...
  return (
    <>
      {/* RealScout Web Component Loader */}
      <Script 
      ...
      <main 
        className={`${jakarta.className} ${playfair.variable} min-h-screen bg-[#10295A] text-white antialiased flex flex-col items-center justify-between px-3 sm:px-4 py-5 selection:bg-[#DB1263]/30`}
      >
        <div className="w-full max-w-sm mx-auto space-y-4">
          ...
          <div className="flex items-center justify-between px-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/10 hover:bg-white text-white hover:text-[#10295A] text-[13px] font-semibold backdrop-blur-sm border border-white/15 transition-all duration-200 shadow-sm active:scale-95 cursor-pointer"
          ...
          {/* MAIN PROFILE & ACTION BUTTON CARD */}
          <div className="w-full bg-white/[0.04] backdrop-blur-md rounded-[2.5rem] p-5 sm:p-6 border border-white/15 shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
            {/* Profile Header */}
          ...
          <div className="flex items-center gap-3 px-4 py-1">
            ...
            <span className="text-[11px] font-semibold uppercase tracking-widest text-[#E6E7E8]/60 font-sans">
              Market Tools & Inventory
            </span>
            <div className="h-px flex-1 bg-white/20" />
          </div>

          {/* HOME VALUATION CARD */}
          <div className="w-full bg-white text-slate-900 rounded-[2rem] p-5 shadow-lg border border-[#E6E7E8] flex flex-col items-center text-center">
            <h3 className="text-base font-semibold text-[#10295A] mb-1 font-['Playfair_Display']">
              {current.cmaTitle}
            </h3>
          ...
          {/* OFFICE LISTINGS 2-COLUMN CAROUSEL CARD */}
          <div className="w-full bg-white text-slate-900 rounded-[2rem] p-4 sm:p-5 shadow-lg border border-[#E6E7E8] overflow-hidden">
            <div className="flex items-center justify-between mb-3 px-1">
              <div>
                <h3 className="text-base font-semibold text-[#10295A] font-['Playfair_Display'] flex items-center gap-2">
                  <Building2 className="w-4 h-4 text-[#1A9175]" />
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-[11px] font-bold text-[#1A9175] hover:text-[#10295A] flex items-center gap-0.5 transition-colors"
              >
                MLS
            ...
            {/* Direct Linkout Footer */}
            <div className="mt-3 pt-3 border-t border-slate-100 text-center">
              <a
                href="https://bernardojimenez.realscout.com/onboarding"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-2 w-full py-2.5 px-4 rounded-xl bg-[#10295A] hover:bg-[#10295A]/90 text-white text-xs font-semibold tracking-wide transition-all shadow-sm active:scale-[0.99]"
              >
                <span>{current.viewAllListings}</span>
                <ArrowUpRight className="w-3.5 h-3.5 text-[#1A9175]" />
              </a>
            </div>
          </div>
        </div>

        {/* Global Styles for RealScout Valuation */}
        <style jsx global>{`
